<template>
 <div class="xiaojiao">
   <div class="yizhi">
     <span>非线性处理:</span>
     <select size="1" id="slected" @change="getNumber(Num3)" v-model="Num3">
       <option v-for="(item,index) in kongList" :value="item.biao" style="height: 10px">{{item.name}}</option>
     </select>
   </div>
   <div class="xiao1">
     <div class="col-lg-12 control-section">
       <div class="slider-content-wrapper">
         <div class="slider_container" style="position: relative">
           <p style="position: absolute;top:20px;left: -90px;color: #fff">关闭</p>
           <p style="position: absolute;top:20px;left: 65px;width: 40px;color: #fff">开启</p>
           <ejs-slider id="gradient_slide" min=0 max=1 value=0 type='MinRange' v-model="AEC"></ejs-slider>
         </div>
       </div>
     </div>
   </div>
 </div>
</template>

<script>
    import Vue from 'vue';
    import { SliderPlugin } from "@syncfusion/ej2-vue-inputs";
    Vue.use(SliderPlugin);
    import { enableRipple } from '@syncfusion/ej2-base';
    enableRipple(true);
    export default {
        name: "xiao-page",
        data: function() {
          return {
            kongList:[{name:'轻微',biao:1},{name:'适中',biao:2},{name:'重度',biao:3}],
            Num3:1,
            AEC:1,
            IP:'',
            response:[]
          };
        },
        watch:{
          AEC:function (val,oldVal) {
            if(document.querySelector('.img20').src!='data:image/png;base64,'){
              let aec = 0
              if(this.AEC!==0){
                aec=this.Num3
              }else{
                aec = 0
              }
              this.response.channel[0].AEC=aec
              localStorage.setItem('response',JSON.stringify(this.response))
              let silence=`<Channel no="0"  AEC="${aec}"></Channel>`
              let encrypted=this.coding(silence)
              this.$.ajax({
                url: `http://${this.IP}`,
                data:{control:1,commd:encrypted},
                type: "POST",
                dataType:'json',
                error:(err)=>{
                  console.log(`<Channel no="0"  AEC="${aec}"></Channel>`)
                  let IP=this.IP
                  let silence1=`<Channel no="0"  AEC="${val}"></Channel>`
                  let encryped1=this.coding(silence1)
                  if(err.statusText=="error"){
                    console.log("出错了")
                    setTimeout(()=>{
                      this.$.ajax({
                        url: `http://${IP}`,
                        data:{control:1,commd:encryped1},
                        type: "POST",
                        dataType:'json',
                      })
                    },1500)
                  }
                }
              })
            }
          }
        },


        methods:{
          getNumber(value){
            if(this.AEC==1){
              if(document.querySelector('.img20').src!='data:image/png;base64,'){
                this.response.channel[0].AEC=this.Num3
                localStorage.setItem('response',JSON.stringify(this.response))
                let silence=`<Channel no="0" AEC="${this.Num3}"></Channel>`
                let encrypted=this.coding(silence)
                this.$.ajax({
                  url: `http://${this.IP}`,
                  data:{control:1,commd:encrypted},
                  type: "POST",
                  dataType:'json',
                  error:(err)=>{
                    console.log(`<Channel no="0" AEC="${this.Num3}"></Channel>`)
                    let IP1=this.IP
                    let silence1=`<Channel no="0" AEC="${this.Num3}"></Channel>`
                    let encryped1=this.coding(silence1)
                    if(err.statusText=="error"){
                      console.log("出错了")
                      setTimeout(()=>{
                        this.$.ajax({
                          url: `http://${IP1}`,
                          data:{control:1,commd:encryped1},
                          type: "POST",
                          dataType:'json',
                        })
                      },1500)
                    }
                  }
                })
              }
            }
          }
        },
        created(){
          this.IP=localStorage.getItem('ip')
          if(document.querySelector('.img20').src!='data:image/png;base64,'){
            if(localStorage.getItem('response')!=undefined){
              this.response=JSON.parse(localStorage.getItem('response'))
              console.log(this.response.channel[0].AEC)
              if(this.response.channel[0].AEC==0){
                this.AEC=0
                this.Num3=1
              }else if(this.response.channel[0].AEC==1){
                this.AEC=1
                this.Num3=1
              }else if(this.response.channel[0].AEC==2){
                this.Num3=2
              }else{
                this.Num3 = 3
              }
            }
          }
        },
    }
</script>

<style lang="less">
  @import "../../node_modules/@syncfusion/ej2-vue-inputs/styles/material.css";
  div.xiaojiao{
    height: 37%;
    position: relative;
    .yizhi{
      position: absolute;
      top:20%;
      left: 41%;
      width: 20%;
      span{
        color: #fff;
        font-size: .9rem;
      }
      select{
        width:25%;
      }
    }
    .xiao1{
      position: absolute;
      top:60%;
      left: 45%;
    }
    p{
      display: flex;
      text-align: center;
      margin-left: 47%;
      span{
        width: 50px;
        color: #fff;
      }
    }
  }
  .slider-content-wrapper {
    width: 2%;
    margin: 0 auto;
    min-width:60px;
  }
  .slider-labeltext {
    text-align: left;
    font-weight: 500;
    font-size: 13px;
    padding-bottom: 10px;
  }
  #gradient_slide .e-handle {
    height:30px;
    width: 30px;
    border-radius: 50%;
    background: url("../assets/core.png");
    border: none;
    background-size:100% 100%;
    border-radius: 0;
    margin-top: 4%;
    top: calc(50% - 10px);
    margin-left:-12px;
  }
  #gradient_slide .e-handle:active{
    transform: scale(1);
  }
  #gradient_slide .e-range {
    height: 20px;
    top: calc(50% - 3px);
    border-radius:10px;
    background-color: #08a2f9;
  }
  #gradient_slide .e-slider-track {
    height: 20px;
    top: calc(50% - 4px);
    border-radius: 10px;
  }
</style>
